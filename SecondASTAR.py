...
class AstarBoard(Board):
    def findMinDistance(self, seed, head):
        ...
        y = min(abs(seed[1] - head[1]) , abs(self.height-abs(seed[1] - head[1])))
        return x+y
    ...
    def guessCost(self, state, alpha):
        ...
        heuristic = -math.inf
        for seed in oneScore:
            distance = self.findMinDistance(seed, headPos)
            if distance > heuristic:
                heuristic = distance
        for seed in twoScore:
            distance = (self.findMinDistance(seed, headPos) + length + 1) // 2
            if distance > heuristic:
                heuristic = distance
        heuristic += len(state.oneScore) + len(state.twoScore) - 1
